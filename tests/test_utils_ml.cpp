...
void test_noise_generators() {
    ...
    std::cout << "OU noise correlation: " << torch::corrcoef(torch::stack({ou_sample1, ou_sample2}))[0][1].item<float>() << std::endl;
...
int main() {
    TestSuite suite("PPOkemon ML Utilities Tests");
    
    // Add test cases
    suite.AddTest("EMA", test_ema);
    suite.AddTest("Advantage Estimator", test_advantage_estimator);
    suite.AddTest("Weight Initializer", test_weight_initializer);
    suite.AddTest("Noise Generators", test_noise_generators);
    ...
    std::cout << "\nFinal Results: " << passed << "/" << total << " utility tests passed" << std::endl;
